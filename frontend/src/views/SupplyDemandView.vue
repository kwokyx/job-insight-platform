...
const effectiveToken = computed(() => {
  const propToken = typeof props.token === 'string' ? props.token.trim() : ''
  if (propToken) return propToken
  return typeof authStore.token === 'string' ? authStore.token.trim() : ''
})
const hasToken = computed(() => Boolean(effectiveToken.value))
...
async function loadData() {
  ...
  try {
    supplyDemandResult.value = await fetchSupplyDemand(effectiveToken.value)
  } catch (error) {
    supplyDemandResult.value = null
...
watch(effectiveToken, (nextToken, prevToken) => {
